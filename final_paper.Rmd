...
Water supply data came from five local water districts, and was provided as part of one of our group members' grou projects (see references). This supply data consists of monthly water volume delivered to each district from each water source. For the streamgraph, data was aggregated across districts within each month. 

The methods for the package are as follows:
...
x <- kable(x, caption = "Groups of randomlly selected values. Each group represents 3 values randomlly selected from a 51 observations data set.", align = "c")
x 
